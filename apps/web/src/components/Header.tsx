...
import { PhoneCall, Search, Sparkles, User, HelpCircle, Phone, Menu, ShieldCheck } from 'lucide-react';
    ...
    <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-2xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-15 flex items-center justify-between gap-4">
        ...
        <div className="hidden md:flex items-center gap-3 flex-1 max-w-xl mx-auto">
          ...
          {activeCall && (
            ...
            >
              ...
              <span className="hidden lg:inline">{activeCall.prospect.businessName}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* UX Rationale Modal Button */}
          <button
            id="header-ux-decisions-btn"
            onClick={onOpenUxModal}
            className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-900 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-xl transition-all cursor-pointer"
          >
            <Sparkles className="w-3.5 h-3.5 text-indigo-600" />
            <span>Decisiones UX/UI</span>
          </button>
