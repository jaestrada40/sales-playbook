...
interface CallResultModalProps {
  ...
  prospect: Prospect;
  initialNotes: string;
  onConfirm: (notes: string, dateStr?: string) => Promise<void>;
  onCancel: () => void;
}
...
export const CallResultModal: React.FC<CallResultModalProps> = ({
  ...
  const [notes, setNotes] = useState(
    initialNotes || "Llamada completada con éxito.",
  );
        ...
        <div className="p-5 space-y-4 text-xs">
          <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
            ...
            <p className="text-slate-500">
              {prospect.contactName} ({prospect.phone})
            </p>
          </div>
          ...
          <div className="pt-2 flex items-center justify-end gap-2">
            ...
            <button
              ...
              onClick={async () => {
                ...
                setError("");
                try {
                  await onConfirm(notes, meetingDate);
                } catch (saveError) {
                  setError(
            ...
            >
              {isSaving ? "Guardando…" : "Guardar y Finalizar Llamada"}
            </button>
          </div>
        </div>
      </div>
