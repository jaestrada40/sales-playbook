...
export const DashboardScreen: React.FC<DashboardScreenProps> = ({
  ...
  const getOutcomeBadge = (outcome: string) => {
    switch (outcome) {
      case 'cita_agendada':
        return { label: 'Cita Agendada', color: 'bg-emerald-100 text-emerald-800 border-emerald-300' };
      case 'interesado':
        return { label: 'Interesado', color: 'bg-indigo-100 text-indigo-800 border-indigo-300' };
      case 'seguimiento':
        return { label: 'Seguimiento Pendiente', color: 'bg-amber-100 text-amber-800 border-amber-300' };
      case 'no_interesado':
        return { label: 'No Interesado', color: 'bg-rose-100 text-rose-800 border-rose-300' };
      default:
        return { label: 'No Contestó', color: 'bg-slate-100 text-slate-700 border-slate-300' };
    }
  };
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-6 sm:p-8 shadow-xl border border-slate-800 relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="relative z-10 max-w-xl space-y-2">
          ...
          <p className="text-sm text-slate-300 leading-relaxed">
            Tienes <span className="text-emerald-400 font-bold">3 prospectos prioritarios</span> listos para llamada sobre migración de terminales POS y tarifarios de procesamiento.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-extrabold text-slate-900">{user.todayCallsCount}</span>
            <span className="text-xs text-emerald-600 font-bold flex items-center gap-0.5">
              +18% vs ayer
            </span>
          </div>
          <p className="text-[11px] text-slate-400">Objetivo diario: 20 llamadas</p>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-extrabold text-slate-900">{user.todayMeetingsBooked}</span>
            <span className="text-xs text-emerald-600 font-bold">5 demos activas</span>
          </div>
          <p className="text-[11px] text-slate-400">Valor potencial: $145,000 USD</p>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-amber-600 font-semibold">Programados hoy</span>
          </div>
          <p className="text-[11px] text-slate-400">Próximo: 11:30 AM (La Casona)</p>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-extrabold text-slate-900">{user.conversionRatePercent}%</span>
            <span className="text-xs text-purple-600 font-bold">+4.2% esta semana</span>
          </div>
          <p className="text-[11px] text-slate-400">Promedio del equipo: 28.5%</p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div>
                ...
                <p className="text-xs text-slate-500">Historial de conversaciones asistidas de hoy</p>
              </div>
              <button
                  ...
                  <div key={log.id} className="p-4 hover:bg-slate-50/80 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-semibold border ${outcomeInfo.color}`}>
                          {outcomeInfo.label}
                        </span>
                      </div>
                    ...
                    <div className="shrink-0 flex items-center gap-3 justify-between sm:justify-end">
                      <div className="text-right">
                        <span className="block text-xs font-semibold text-slate-700">Duración</span>
                        <span className="text-xs font-mono text-slate-500">{formatDuration(log.durationSeconds)}</span>
                      </div>
                      <button
                        onClick={onStartNewCall}
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs p-5 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">Guiones de ventas más efectivos</p>
              </div>
              <button
          ...
          <div className="p-4 bg-indigo-950 text-white rounded-2xl shadow-md space-y-2 border border-indigo-900">
            ...
            <p className="text-xs text-indigo-100 leading-relaxed">
              Mencionar la <strong className="text-white">SIM 4G Multi-carrier ilimitada incluida</strong> en la Clover Flex reduce la objeción por caídas de Wi-Fi en un 64%.
            </p>
          </div>
        </div>
      </div>
